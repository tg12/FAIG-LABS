...
def place_order(pred_ict):
    ...
    if float(std_err) > -2 and float(std_err) < 2:
        ...
        vol_avg = np.mean(last_traded_volume)
        
        if price_diff > 0 and float(shortPositionPercentage) > Client_Sentiment_Check and int(last) < int(vol_avg):
            DIRECTION_TO_TRADE = "SELL"
            limitDistance_value = str(int(float(price_diff) * float(slope) * float(greedy_trader)))
        elif price_diff < 0 and float(longPositionPercentage) > Client_Sentiment_Check and int(last) > int(vol_avg):
            DIRECTION_TO_TRADE = "BUY"
            limitDistance_value = str(int(float(price_diff) * float(slope) * float(greedy_trader)))
            limitDistance_value = str(int(limitDistance_value * -1)) #Format for IG
        else:
    ...
    if float(stopDistance_value) < float(limitDistance_value):
        stopDistance_value = float(limitDistance_value) * cautious_trader
...
for x in range(0, 9999):
    ...
    if now_time >= time(14,30) or now_time <= time(20,30):
        print ("NY OPEN")
